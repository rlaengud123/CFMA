...
class Conv_block1D(torch.nn.Module):
    def __init__(self, blocks):
        super().__init__()
        self.blocks = blocks
        self.kernel_sizes = [16, 8, 4]
        self.layer1 = self.make_layer(self.kernel_sizes[0])
        self.layer2 = self.make_layer(self.kernel_sizes[1])
        self.layer3 = self.make_layer(self.kernel_sizes[2])
        self.GAP = torch.nn.AdaptiveAvgPool1d(1)
    
    def make_layer(self, kernel_size):
        layers = []
        for i in range(len(self.blocks)-1):
            layers.append(torch.nn.Conv1d(self.blocks[i], self.blocks[i+1], kernel_size, 1, 0, 1))
            layers.append(torch.nn.BatchNorm1d(self.blocks[i+1], affine=self.training))
            layers.append(torch.nn.ReLU())
        layers.append(torch.nn.Conv1d(self.blocks[i+1], 1, kernel_size, 1, 0, 1))
        layers.append(torch.nn.BatchNorm1d(1, affine=self.training))
    ...
    def forward(self, x):
        ...
        x2 = self.layer2(x2)
        x3 = self.layer3(x3)
        x1 = self.GAP(x1).reshape(x.shape[0], -1)
        x2 = self.GAP(x2).reshape(x.shape[0], -1)
        x3 = self.GAP(x3).reshape(x.shape[0], -1)
